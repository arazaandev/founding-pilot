      ...
      <main>
        <section className="sample-hero">
          <div className="container">
            ...
            <div className="sample-summary">
              <article><span>Opportunity pattern</span><strong>{opportunity.pattern.name}</strong><p>{opportunity.pattern.summary}</p></article>
              <article><span>Commercial potential</span><strong>{opportunity.commercialPotential.level}</strong><p>{opportunity.commercialPotential.summary}</p></article>
              <article><span>Evidence confidence</span><strong>{opportunity.evidenceConfidence.level}</strong><p>{opportunity.evidenceConfidence.summary}</p></article>
            </div>
          </div>
        </section>
        ...
        <section className="section tint">
          <div className="container">
            ...
            <div className="timeline-evidence">
              {opportunity.evidence.map((item) => <article key={item.type}><div className="evidence-meta"><span>{item.type}</span><span>Observed {item.observedDate}</span><span>Last checked {item.checkedDate}</span></div><h3>{item.observed}</h3><p className="muted">Source unavailable in this fictional demonstration.</p></article>)}
            </div>
          </div>
        </section>
        ...
        <section className="section container">
          ...
          <div className="decision-grid">
            <article><span className="tag">Commercial potential</span><strong className="metric-word">{opportunity.commercialPotential.level}</strong><p>The potential project may suit AwanData’s delivery capabilities, reference proof, geography, and preferred contract profile. The assessment is qualitative because the model is not customer validated.</p><b>What could reduce it</b><p>{opportunity.commercialPotential.limitation}</p></article>
            <article><span className="tag">Evidence confidence</span><strong className="metric-word">{opportunity.evidenceConfidence.level}</strong><p>Three recent and related fictional source types support the change pattern. None confirms project ownership, urgency, budget, external-support need, or partner status.</p><b>Why it is not high</b><p>{opportunity.evidenceConfidence.limitation}</p></article>
          </div>
        </section>
        ...
        <section className="section tint">
          <div className="container">
            ...
            <div className="stakeholder-grid">{opportunity.stakeholders.map((person) => <article key={person.role}><span>{person.order}</span><h3>{person.role}</h3><p>{person.reason}</p><b>Conversation objective</b><p>{person.objective}</p></article>)}</div>
          </div>
        </section>
        ...
        <section className="section container">
          ...
          <div className="unknown-grid">{opportunity.unknowns.map(([title, copy]) => <article key={title}><span className="tag">Unknown</span><h3>{title}</h3><p className="muted">{copy}</p></article>)}</div>
        </section>
